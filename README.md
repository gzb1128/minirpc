...
上面那张时序图只画了"client/server 进程"两层。但要真正理解流式 RPC,得把同一时刻的**四个层次**拆开看 —— 这正是为什么 client 和 server 会**自己**在方法里处理流式 frame:

```
以 Demo 3 的 server-streaming 为例:server 在一条流上推 progress DATA

时间 ──►

┌─ TCP 字节流(物理一层)──────────────────────────────────────────────────────┐
│  一条管子,所有 frame 是字节流,没有消息边界。框架不直接读业务,而是先切帧。   │
│  线上此刻是:[len][id=B][DATA][33%...][len][id=B][DATA][66%...][len]...     │
│  对框架来说它只是字节 —— 框架关心"这一段是不是一个完整 frame",不关心内容。  │
└──────────────────────────────────────────────────────────────────────────────┘
        │ readLoop 不停 ReadFrame 切帧
        ▼
┌─ 逻辑流(StreamID 分发,框架的活)──────────────────────────────────────────┐
│  readLoop 切出帧后,只看 header 里的 StreamID,投递到对应入站队列:           │
│                                                                            │
│    stream#A 的队列:[REQUEST] ... [RESPONSE]    ← 主调用(Import)           │
│    stream#B 的队列:[DATA][DATA][DATA][CLOSE]   ← progress 流                │
│                                                                            │
│  ⚠ 框架到这里就"交付"完了:把帧塞进队列,谁注册了谁去 Recv。               │
│    它**不解析** DATA 里装的是 progress 还是镜像层还是别的什么。             │
└──────────────────────────────────────────────────────────────────────────────┘
        │ stream.Recv() 取走帧
        ▼
┌─ 框架认识的信号(协议级,框架统一处理)──────────────────────────────────┐
│  框架只认两种 frame 并采取行动:                                            │
│    REQUEST  → 分派到 handler(serveStream 拿 method + args 起方法)         │
│    CLOSE    → 转成 io.EOF(表示这个方向的流结束,唤醒阻塞的 Recv)          │
│  这两件事是"框架的职责",无论 unary 还是 streaming 都一样,代码一致。       │
└──────────────────────────────────────────────────────────────────────────────┘
        │ 剩下的 DATA 帧,框架只是搬运,不解语义
        ▼
┌─ function 自己协商的协议(框架无关)──────────────────────────────────────┐
│  DATA 帧里装什么、传几帧、往哪个方向传 —— 全靠 client 和 server 在这个      │
│  method 上的约定。框架完全不插手:                                         │
│                                                                            │
│    Demo 3 的约定:server handler 自己 Send({event:progress, value:33}),    │
│    client goroutine 自己 Recv 后 json.Unmarshal 成 Progress。              │
│    这个 {event,value} 结构是 ImportService 的接口契约,不是 minirpc 的。    │
└──────────────────────────────────────────────────────────────────────────────┘
```

**为什么 client/server 会"自己"在方法里处理流式 frame?**

因为这不是 RPC 框架的一种"模式",而是**框架抽象天然支持的副作用**:
- 框架提供了"一条逻辑流 + readLoop 按 StreamID 分发 + 队列"这一层。
- 只要帧符合 minirpc 的格式(长度前缀 + StreamID + Type),框架就负责把它完整地、
  按序地送到对端对应的逻辑流上。
- **流式传输就是在这条已建立的流上,额外附加上一段 client/server 协商好的协议**:
  "我接下来发的 DATA 你按 X 格式解"。这段协议长什么样,和框架无关。
- 框架只保证"帧是完整的、送达对的流";帧里业务字段怎么编排、怎么对账,
  是 function 的事。

所以:**用不用流式、流上格式怎么定 —— 都和框架无关。**框架修的是"路"(逻辑流),
流式传输是上面跑的"车",车的设计是车主(client/server function)自己的事。
